...
GRAB.Marker <- function(objNull,
  ...
  if (!Start) {
    message <- paste0(
      "We detected that parts of analysis have been conducted from file:\t",
      OutputFileIndex, "\n",
  ...
  for (i in (indexChunk + 1):nChunks)
  {
    ...
    if (NullModelClass == "WtCoxG_NULL_Model") {
      markerInfo_chunk <- markerInfo[match(genoIndex, markerInfo$genoIndex), ]
      resMarker <- mainMarker.WtCoxG.inR(genoType, markerInfo_chunk, control, objNull)
    }
...
mainMarker <- function(NullModelClass, genoType, genoIndex, outputColumns, objNull, obj.setMarker) {
  ...
  # Check 'WtCoxG.R'
  if (NullModelClass == "WtCoxG_NULL_Model") {
    obj.mainMarker = mainMarker.WtCoxG(genoType, genoIndex, outputColumns, obj.setMarker)
  }
